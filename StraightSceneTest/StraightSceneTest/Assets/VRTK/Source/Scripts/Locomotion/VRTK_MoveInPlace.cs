...
namespace VRTK
{
    ...
    public class VRTK_MoveInPlace : MonoBehaviour
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            headset = VRTK_DeviceFinder.HeadsetTransform();//头戴式设备获得当前位置

            SetControlOptions(controlOptions);

            playArea = VRTK_DeviceFinder.PlayAreaTransform();//使用的物理空间的范围
        ...
        protected virtual float CalculateListAverage() //计算y方向上的移动
        {
            float listAverage = 0;
        ...
        protected virtual void MovePlayArea(Vector3 moveDirection, float moveSpeed)//是想要的函数，x,z,移动Y不变
        {
            Vector3 movement = (moveDirection * moveSpeed) * Time.fixedDeltaTime;//某段时间内发生的位移movement
            if (playArea != null)
            {
