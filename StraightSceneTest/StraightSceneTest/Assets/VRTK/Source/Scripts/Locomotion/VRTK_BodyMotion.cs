namespace VRTK
{
    ...
    [AddComponentMenu("VRTK/Scripts/Locomotion/VRTK_BodyMotion")]
    public class VRTK_BodyMotion : MonoBehaviour
    {
       ...
       // protected float currentMovement;
        ...
        protected virtual void FixedUpdate()//注意
        {
            ...
            if (MovementActivated() && !currentlyFalling) //①当点击手柄，激活按钮开始运动
            {
                ...
                float motion = CurrentBodyMotion(UsingBody);//当前使用的部位
                SumX = SumX + motion;
                previousDirection = currentDirection;
                currentDirection = SetDirection();
                currentMovement = motion; //③得到运动输入
            }
            else
            {
                currentMovement = 0f;
       ...
       protected virtual int RandomDistanceAndMapping()//随机距离和映射函数的组合
        {
            int num = UnityEngine.Random.Range(1,13);//3种距离和4种映射关系的组合为12，重复两次即24

            if (randomNumList.Count == 12) //检查是否已经随机过
            {
                randomNumList.Clear();
            }
            while (CheckRandom(num)) //检查随机的数字是否存在于已有列表
            {
        ...
        protected virtual int MappingBasedRandom(int randomNum)//根据随机数判断使用的映射函数,有12个数,函数有4种,因此需要除以5,余数则是目前使用的函数
        {
            int mappingBasedRandom = -1;

            mappingBasedRandom = randomNum % 4;
            if (mappingBasedRandom == 0) //若可以整除,则使用的是函数4
            {
                mappingBasedRandom = 4;
            }
            return mappingBasedRandom;
        }

        protected virtual string TargetBasedRandom(int randomNum)//根据随机数判断使用的目标距离(1-6为A点，7-12为B点，13-18为C点)
        {
        ...
        protected virtual void RandomEvent()
        {
            ...
            {
                a = RandomDistanceAndMapping(); //正式实验使用这个函数
            }
            ...
            {
                case 1: MappingDisplay = "线性函数"; break;
                //  case 2: MappingDisplay = "线性1.8"; break;
              //  case 2: MappingDisplay = "2次幂函数"; break;
                 case 2: MappingDisplay = "幂函数"; break;
               // case 3: MappingDisplay = "分段线性函数"; break;
                case 3: MappingDisplay = "分段函数(常数+最小值函数)"; break;
            ...
            {
                case "A": ArrowZ = 5; break;
                case "B": ArrowZ = 10; break;
                case "C": ArrowZ = 15; break;
                default: break;
            }
            ...
            Arrow.transform.position = new Vector3(0, 2, ArrowZ);
        ...
        protected virtual float CalculateAngleUpdate()//使用膝盖交互
        {
           ...
           // Debug.Log("I5" + tracker1.transform.localPosition + "I6" + tracker2.transform.localPosition + "之前的向量：" + preTrackerV3 + "现在的向量：" + nowTrackerV3 + "计算得到的角度" + angle);
            ...
            //    angle = angle - 0.1f;
            //}
            if (angle < 0)//假设两个控制器之间的距离小于0.4的距离，默认不移动，当大于0.4把超出的距离做函数映射
            {
                angle = 0;
            }
        ...
        protected virtual float CalculateTorsoAngle()//计算躯干角度
        {
            ...
            float displacement= angle / 20f; //规范化过程
        ...
        {
            //float distance = 0f;
            // float mappingDistance = 0f;
            // distance = moveDistance;
            //if (moveDistance > 0) //当输入距离大于0才进行映射
            //{
        ...
        protected virtual void TrialRedoCheck()
        {
        ...
        protected virtual void DataRecordMovementtoSpeed() //数据记录映射前的运动，映射后的速度之间的关系
        {
            ...
            using (excelPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i <bodyMotion.Count; i++)
                {
                    ...
                    worksheet.Cells[endRow, 7].Value = bodyMotion[i];  //运动输入
                    worksheet.Cells[endRow, 8].Value = mappingSpeed[i];  //速度
                    worksheet.Cells[endRow, 9].Value = timeCost[i]; //任务完成时间 
                }

                if (IsNeedRedoTask) //如果需要重做任务就是无效的，不需要重做任务就是有效的
                {
                    worksheet.Cells[endRow, 10].Value = "InValid"; //任务完成时间
                }
                else
                {
                    worksheet.Cells[endRow, 10].Value = "Valid"; //任务完成时间
                }
        ...
        protected virtual void EngageButtonPressed(object sender, ControllerInteractionEventArgs e)
        {
            ...
            if (IsNeedRedoTask)
            {
                ...
                StartCoroutine(RecordDataPoint(0.04f)); //开始携程记录用户进行一次trial期间的采样点
                IsPressReset = false;
            } //是否需要重做任务
            else
            {
                if (IsDoneTask)
                {
                    ...
                    {
                        if (recordNum > 24) //如果大于48次直接return，不允许开始
                        {
