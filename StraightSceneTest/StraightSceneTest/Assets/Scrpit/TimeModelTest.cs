    ...
    public class TimeModelTest : MonoBehaviour
    {
        ...
        protected virtual float CurrentBodyMotion(int randomNum) //当前使用的肢体部位
        {
            ...
            switch (randomNum)
            {
                ...
                case 2: returnDis = CalculateArmAngle(); break;//case2手部(角度变化)
        ...
        {
            int num = UnityEngine.Random.Range(1, 21);//5种距离和4种映射关系的组合为20，重复两次即40 ,随机数应该是1到20

            if (randomNumList.Count == 20) //检查是否已经随机过
            {
                randomNumList.Clear();
            }
            while (CheckRandom(num))
            {
                num = UnityEngine.Random.Range(1, 21);
                CheckRandom(num);
            }
        ...
        protected virtual int MappingBasedRandom(int randomNum)//根据随机数判断使用的映射函数（除以5）
        {
            ...
            mappingBasedRandom = randomNum % 4;  //使用的是除数不是余数
        ...
        protected virtual void EngageButtonPressed(object sender, ControllerInteractionEventArgs e)
        {
            ...
            else
            {
                if (IsDoneTask)
                {
                    ...
                    {
                        if (recordNum > 20) //如果大于48次直接return，不允许开始 4种函数,3种距离,重复2次,现在是4种函数,5种距离,重复两次,共40
                        {
                ...
                else
                {
                    ...
                    {
                        if (recordNum > 20) //如果大于48次直接return，不允许开始
                        {
                            tips.text = "您已完成所有组合的测试。";
                            return;
                        }
